...
try:
    from userscope.__init__ import import_error_test_var # noqa: F401
except ImportError:
    print("UserScope engine requires the 'userscope' package.")
    print("Install it via 'pip install -e .' from the project folder.")
    sys.exit(1)
...
from userscope.__init__ import (
    __longname__,
    __shortname__,
    __version__,
...
from userscope.result import QueryResult
from userscope.notify import QueryNotify
from userscope.notify import QueryNotifyPrint
from userscope.sites import SitesInformation
from colorama import init
from argparse import ArgumentTypeError


class FuturesSessionWithHooks(FuturesSession):
    def request(self, method, url, hooks=None, *args, **kwargs):
        ...
        return super(FuturesSessionWithHooks, self).request(
...
def check_for_parameter(username):
    ...
    if exists, UserScope is looking for multiple usernames"""
    return "{?}" in username
...
def search(
    ...
    """Run a UserScope username analysis.
    ...
    session = FuturesSessionWithHooks(
    ...
    for social_network, net_info in site_data.items():
        ...
        # As WAFs advance and evolve, they can occasionally block probing and
...
def main():
    ...
    # Check for a newer engine version and let the user know.
    try:
